...
class Toplevel(board: String, invReset: Boolean = true) extends Module {
  ...
  withClockAndReset(pll.io.clko, customReset) {
    // Instantiate the Blink module using 50Mhz from PLL output
    val bl = Module(new Blinky(50000000))
...
object Toplevel extends App {
  ...
  val board: String =
    params.getOrElse("board",
    throw new IllegalArgumentException("The '-board' argument should be informed."))
